...
export function DashboardApp() {
  ...
  useEffect(() => { const frame = window.requestAnimationFrame(() => { const loaded = loadPlanner(window.localStorage, samplePlanner); if (!loaded.planner) { setLoadState("empty"); return; } let next = loaded.planner; let resetApplied = false; if (window.localStorage.getItem(LIVE_TRIAL_WEEK_RESET_KEY) !== "applied") { const reset = applyLiveTrialWeekReset(next); if (reset.applied) { next = reset.planner; persistPlanner(window.localStorage, next); window.localStorage.setItem(LIVE_TRIAL_WEEK_RESET_KEY, "applied"); resetApplied = true; } } setPlanner(next); setLoadState("ready"); if (loaded.source !== "v9" || resetApplied) { persistPlanner(window.localStorage, next); setMigrationNotice(true); } }); return () => window.cancelAnimationFrame(frame); }, []);
