...
export type CloudWorkspaceControls = {
  planner: PlannerData;
  user: { displayName: string; email: string; initials: string };
  sync: { label: string; tone: "synced" | "saving" | "offline" | "pending" | "conflict" };
  onPlannerChange: (planner: PlannerData) => void;
  onImportBackup: (planner: PlannerData) => Promise<PlannerData>;
...
export function DashboardApp({ cloud }: { cloud?: CloudWorkspaceControls } = {}) {
  const isCloud = Boolean(cloud);
  const [planner, setPlannerState] = useState<PlannerData | null>(cloud?.planner ?? null);
  const [loadState, setLoadState] = useState<"loading" | "empty" | "recovery" | "ready">(cloud ? "ready" : "loading");
  const [recoveryError, setRecoveryError] = useState("");
  const recoveryImportRef = useRef<HTMLInputElement>(null);
  ...
  function setPlanner(next: PlannerData) {
    ...
    cloud?.onPlannerChange(next);
  }

  useEffect(() => { if (isCloud) return; const frame = window.requestAnimationFrame(() => { const loaded = loadPlanner(window.localStorage, samplePlanner); if (!loaded.planner) { setLoadState(loaded.source === "invalid-v9" ? "recovery" : "empty"); return; } let next = loaded.planner; let resetApplied = false; let sessionLabelMigrated = false; let classCoverMigrated = false; if (window.localStorage.getItem(LIVE_TRIAL_WEEK_RESET_KEY) !== "applied") { const reset = applyLiveTrialWeekReset(next); if (reset.applied) { next = reset.planner; persistPlanner(window.localStorage, next); window.localStorage.setItem(LIVE_TRIAL_WEEK_RESET_KEY, "applied"); resetApplied = true; } } if (window.localStorage.getItem(SESSION_ONE_LABEL_MIGRATION_KEY) !== "applied") { const firstSession = next.sessionSlots.find((slot) => slot.id === "session-1"); if (firstSession?.label === "Session 1") { next = touchPlanner({ ...next, sessionSlots: next.sessionSlots.map((slot) => slot.id === "session-1" ? { ...slot, label: "S1" } : slot) }); sessionLabelMigrated = true; } window.localStorage.setItem(SESSION_ONE_LABEL_MIGRATION_KEY, "applied"); } if (window.localStorage.getItem(GENERALIST_COVER_MIGRATION_KEY) !== "applied") { const migration = migrateClassCoverToGeneralistTeaching(next); next = migration.planner; classCoverMigrated = migration.applied; window.localStorage.setItem(GENERALIST_COVER_MIGRATION_KEY, "applied"); } setPlannerState(next); setLoadState("ready"); if (loaded.source !== "v9" || resetApplied || sessionLabelMigrated || classCoverMigrated) { persistPlanner(window.localStorage, next); setMigrationNotice(true); } }); return () => window.cancelAnimationFrame(frame); }, [isCloud]);
  useEffect(() => { if (!isCloud && loadState === "ready" && planner) try { persistPlanner(window.localStorage, planner); } catch { /* Keep the last valid local state during edits. */ } }, [planner, loadState, isCloud]);
  ...
  async function restoreRecoveryBackup(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setRecoveryError(error instanceof Error ? error.message : "This backup could not be restored.");
    }
  }
  ...
  function downloadUnreadableData() {
    ...
    link.href = url;
    link.download = `specialist-planner-unreadable-${new Date().toISOString().slice(0, 10)}.json`;
    link.click();
  ...
  function saveQuickNote() { if (!planner || !quickNote || !quickNoteText.trim()) return; setPlanner(touchPlanner({ ...planner, trialNotes: [{ id: makeId("note"), text: quickNoteText.trim(), createdAt: new Date().toISOString(), context: quickNote.context, classId: quickNote.classId }, ...planner.trialNotes] })); setQuickNote(null); setQuickNoteText(""); }
  function shiftSelected(delta: number) { if (!planner || !selectedClass || !selectedUnit) return; const target = Math.min(Math.max(selectedPosition + delta, 1), selectedUnit.lessons.length); setPlanner(setClassLesson(planner, selectedClass.id, selectedUnit.lessons[target - 1].id)); }

  return <div className="app-shell"><header className="topbar"><button className="brand brand-button" type="button" onClick={() => setView("week")} aria-label="Open Week"><span className="brand-mark">SP</span><span><strong>Specialist Planner</strong><small>{activeSubject.name || "Untitled subject"}</small></span></button><nav aria-label="Primary navigation"><button className={`nav-link ${view === "week" ? "active" : ""}`} type="button" onClick={() => setView("week")}>Week</button><button className={`nav-link ${view === "progress" ? "active" : ""}`} type="button" onClick={() => setView("progress")}>Progress</button><button className={`nav-link ${view === "units" ? "active" : ""}`} type="button" onClick={() => setView("units")}>Units</button><span className="nav-divider" /><button className={`nav-link nav-secondary ${view === "history" ? "active" : ""}`} type="button" onClick={() => setView("history")}>History</button><button className={`nav-link nav-secondary ${view === "settings" ? "active" : ""}`} type="button" onClick={() => setView("settings")}>Settings</button></nav><div className="topbar-actions"><button className="quick-note-button" type="button" onClick={() => openQuickNote(undefined, view === "week" ? "Week" : view === "progress" ? "Progress" : view === "units" ? "Units" : view === "history" ? "History" : "Settings")}>＋ Quick note</button>{cloud && <details className="account-menu"><summary aria-label={`Account menu for ${cloud.user.displayName}`}><span className="avatar">{cloud.user.initials}</span><span><strong>{cloud.user.displayName}</strong><small>{cloud.user.email}</small></span></summary><div><span>Signed in as</span><strong>{cloud.user.displayName}</strong><small>{cloud.user.email}</small><button type="button" onClick={() => void cloud.onSignOut()}>Sign out</button></div></details>}</div></header>
