...
export function DashboardApp() {
  ...
  useEffect(() => { const frame = window.requestAnimationFrame(() => { const loaded = loadPlanner(window.localStorage, samplePlanner); if (!loaded.planner) { setLoadState("empty"); return; } let next = loaded.planner; let resetApplied = false; let sessionLabelMigrated = false; let classCoverMigrated = false; if (window.localStorage.getItem(LIVE_TRIAL_WEEK_RESET_KEY) !== "applied") { const reset = applyLiveTrialWeekReset(next); if (reset.applied) { next = reset.planner; persistPlanner(window.localStorage, next); window.localStorage.setItem(LIVE_TRIAL_WEEK_RESET_KEY, "applied"); resetApplied = true; } } if (window.localStorage.getItem(SESSION_ONE_LABEL_MIGRATION_KEY) !== "applied") { const firstSession = next.sessionSlots.find((slot) => slot.id === "session-1"); if (firstSession?.label === "Session 1") { next = touchPlanner({ ...next, sessionSlots: next.sessionSlots.map((slot) => slot.id === "session-1" ? { ...slot, label: "S1" } : slot) }); sessionLabelMigrated = true; } window.localStorage.setItem(SESSION_ONE_LABEL_MIGRATION_KEY, "applied"); } if (window.localStorage.getItem(GENERALIST_COVER_MIGRATION_KEY) !== "applied") { const migration = migrateClassCoverToGeneralistTeaching(next); next = migration.planner; classCoverMigrated = migration.applied; window.localStorage.setItem(GENERALIST_COVER_MIGRATION_KEY, "applied"); } setPlanner(next); setLoadState("ready"); if (loaded.source !== "v9" || resetApplied || sessionLabelMigrated || classCoverMigrated) { persistPlanner(window.localStorage, next); setMigrationNotice(true); } }); return () => window.cancelAnimationFrame(frame); }, []);
